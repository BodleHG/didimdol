...
class manufacturer:
    ...
    def manufacturer_action(self):
        ...
        if needs_replacement and self.first_replacement_hour is None:
            self.first_replacement_hour = self.production_time / self.time_step
    ...
    def final_replacement_report(self):
        ...
        if self.first_replacement_hour is not None:
            # 해당 시점의 entry를 가져옴
            reason_entry = self.production_log[int(self.first_replacement_hour)]
            ...
            if defect_reason and speed_reason:
                reason_text = "불량률과 생산속도 모두 하락"
            elif defect_reason:
                reason_text = "불량률이 기준치를 초과"
            elif speed_reason:
